          ...
          output$phyloseq <- renderText(
            ...
            if (any(mustHave %in% phyloSlots())){
              ...
              output$unique <- renderText(
                ...
                paste0("There are ", length(get_taxa_unique(saveData(), taxonomic.rank = "Genus")), " unique Genus and ", length(get_taxa_unique(saveData(), taxonomic.rank = "Species")), " unique Species in this phyloseq." )
              )
